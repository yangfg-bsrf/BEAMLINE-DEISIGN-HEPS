...
if True:
    ...
    C1 = Collimator([20.3e3 + sr.center[0], 0], 20, 300, 300, mr2, 'C1')#位置、内口径、外口经、厚度、3*Moliere_radius，name；外口径作为参考口径
    C2 = Collimator([26.35e3 + sr.center[0], 0], 20, 130, 300, mr2, 'C2')
    C3 = Collimator([27.04e3 + sr.center[0], 0], 30, 100, 200, mr2, 'C3 & SS')
    C4 = Collimator([27.8e3 + sr.center[0], 0], 20, 1000,700, mr2, 'C4 & Ratched Wall')
    C5 = Collimator([35.8e3, 0], 6, 130, 200, mr1, 'C5')  #
    C6 = Collimator([45.2e3-150, 42.5], 15, 250, 300, mr2, 'C6')
...
if True:
    #光轴
    plt.plot(centers[:,0], centers[:,1], '-.', color = 'lawngreen',linewidth = 1.5)
    plt.plot(centers_DCM[:,0], centers_DCM[:,1], '-.',color = 'darkorchid', linewidth = 1.5)

    #横坐标ticks位置
    labelx = []
    xticks.append(max(x))
    for item in xticks:
        labelx.append('{0:.2f}m'.format(item*1e-3))   
    plt.xticks(xticks,labelx,rotation='vertical')
           
    labely = []
    yticks = np.linspace(y_limit[0],y_limit[1],11)
    for item in yticks:
        labely.append('{0:.0f}mm'.format(item))   
    plt.yticks(yticks,labely,rotation='horizontal')
...
plt.show()
plt.savefig('Radiation_protection_vertical.png', dpi =300)
