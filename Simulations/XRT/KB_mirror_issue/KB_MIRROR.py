...
def defocus_plot(Plots, dqs): 
    ...
    ax1.set_ylabel(u'FWHM size (nm)', fontsize=14)
    plt.legend()          
    plt.show()
    ...
    ax1.set_xlabel(u'Shift (mm)', fontsize=14)
    ax1.set_ylabel(u'Position (nm)', fontsize=14)
    plt.legend()          
    plt.show()
...
""" ********************全局角度******************** """
accept_ang_x, accept_ang_z = 5e-6, 5e-6

""" 光源 """
Indicator_source = 1
if Indicator_source == 1:
    E0, n0 = 12400, 1  # 插入间工作能量点，及谐波级次
    n1 = 1  # 待分析谐波的能量点
    E1 = E0/n0*n1  # 待分析的能量点
    nrays, repeats = 5e5, 1
    what = 'rays'
...
#                material = Si,
    ...
    kwargs_OE = dict(
                ...
                rotationSequence='RyRzRx',                                    
#                material = Si,
                limPhysX=[-0.4, 0.4], limPhysY=[-150, 150],
...
if Indicator_plot == 1:
    ...
    plot = xrtp.XYCPlot(
        'Samp', aspect='auto',
        xaxis=xrtp.XYCAxis(r'$x$', unit=r"nm", bins=xbins, ppb=xppb),
        yaxis=xrtp.XYCAxis(r'$z$', unit=r"nm", bins=zbins, ppb=zppb),
        caxis=xrtp.XYCAxis('energy', 'eV', offset=E0, limits=[eMin0-1, eMax0+1]),
        title='Sample')    
    plot.baseName = plot.title
    plots.append(plot)
...
#    plot.yaxis.limits = [-z_lim, z_lim]   
    ...
    for i, dq in enumerate(dqs):
        plot = xrtp.XYCPlot(
            'beamFSMn_{0:02d}'.format(i), #(1,),
            xaxis=xrtp.XYCAxis(
                r'$x$', u'nm', bins=xbins, ppb=xppb),#
            yaxis=xrtp.XYCAxis(
                r'$z$', u'nm', bins=zbins, ppb=zppb),#
